...
## Problem definition
We consider a road network graph ```math G = \langle V, E \rangle```, where ```math V``` is a set of ```math n``` vertices  (road intersections)  and ```math E``` is a set of ``` math m``` edges (roads). 
